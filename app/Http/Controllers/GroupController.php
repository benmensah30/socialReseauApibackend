...
class GroupController extends Controller
{
    ...
    {
        $group = $this->groupInterface->show_group();

        return ApiResponses::sendResponse(true, [new GroupResource($group)], "Operation effectuer");
    }
    ...
    public function uploadFile(Request $request)
    {
        ...
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            ...
            $file = $fileReq->store('services', 'public');
            $filePath = $file;
    ...
    {
        $files = $this->groupInterface->show_files($goupId);

        if ($files)
            return ApiResponses::sendResponse(true, [new FileResource($files)], "Operation effectuer");
        else
            return ApiResponses::sendResponse(false, [], "erreur");
    }
}
